...
export class AgentService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    this.conversationModel = connection.model<ConversationDocument>(
      'Conversation',
      ConversationSchema,
    );
  }
  ...
  private async handleCollectInfo(
    conversation: ConversationDocument,
    userMessage: string,
  ): Promise<ChatResponse> {
    ...
    if (missingInfo.length > 0) {
      ...
      const aiMessage = await this.generateInfoCollectionResponse(
        userMessage,
        missingInfo,
        extractedInfo,
      );
  ...
  private async handleRecommendMajors(
    conversation: ConversationDocument,
    userMessage: string,
  ): Promise<ChatResponse> {
    if (this.isPositiveResponse(userMessage)) {
      const recommendations = this.generateMajorRecommendations(
        conversation.userProfile,
      );

      // Sử dụng OpenAI để tạo response tự nhiên
      const aiMessage = await this.generateMajorRecommendationResponse(
        userMessage,
        recommendations,
        conversation.userProfile,
      );
...
Hãy nhập số thứ tự (1, 2, 3) hoặc tên ngành học.
      ...
      return {
        ...
        suggestions: recommendations.map(
          (rec, index) => `${index + 1}. ${rec.name}`,
        ),
      };
    }

    // Xử lý chọn ngành
    const selectedMajor = this.extractMajorSelection(
      userMessage,
      conversation.recommendations?.majors || [],
    );
...
Bạn có muốn tiếp tục không?`,
    ...
    return {
      message:
        'Tôi không hiểu lựa chọn của bạn. Vui lòng nhập số thứ tự (1, 2, 3) hoặc tên ngành học chính xác.',
      phase: 'recommend-majors',
      suggestions:
        conversation.recommendations?.majors?.map(
          (rec, index) => `${index + 1}. ${rec.name}`,
        ) || [],
    };
  }

  private async handleRecommendSchools(
    conversation: ConversationDocument,
    userMessage: string,
  ): Promise<ChatResponse> {
    if (this.isPositiveResponse(userMessage)) {
...
Hãy nhập số thứ tự (1, 2, 3) hoặc tên trường.
      ...
      return {
        ...
        suggestions: schoolRecommendations.map(
          (school, index) => `${index + 1}. ${school.name}`,
        ),
      };
    }

    // Xử lý chọn trường
    const selectedSchool = this.extractSchoolSelection(
      userMessage,
      conversation.recommendations?.schools || [],
    );
    ...
    if (selectedSchool) {
      ...
      await this.legalService.initializeLegalDocumentsForUser(
...
Bạn có câu hỏi gì không?`;
      ...
      return {
        ...
        suggestions: [
          'Kiểm tra tiến độ',
          'Hướng dẫn làm Passport',
          'Làm giấy tờ nào trước?',
        ],
      };
    }
    ...
    return {
      message:
        'Tôi không hiểu lựa chọn của bạn. Vui lòng nhập số thứ tự (1, 2, 3) hoặc tên trường chính xác.',
      phase: 'recommend-schools',
      suggestions:
        conversation.recommendations?.schools?.map(
          (school, index) => `${index + 1}. ${school.name}`,
        ) || [],
    };
  }

  private async handleLegal(
    conversation: ConversationDocument,
    userMessage: string,
  ): Promise<ChatResponse> {
    ...
    if (documentUpdate) {
      try {
        ...
        const progress = await this.legalService.getDocumentProgress(
          conversation.userId.toString(),
        );

        // Sử dụng OpenAI để tạo response tự nhiên
        const aiMessage = await this.generateLegalDocumentResponse(
          userMessage,
          documentUpdate,
          progress,
        );
...
**Tiến độ hiện tại: ${progress.completed}/${progress.total} (${progress.progress}%)**`;
        ...
        if (progress.pending > 0) {
          const pendingDocs = await this.legalService.getPendingDocuments(
            conversation.userId.toString(),
          );
...
Bây giờ bạn có thể tiến hành nộp hồ sơ du học. Chúc bạn thành công!`;
        ...
        return {
          ...
          suggestions:
            progress.pending > 0
              ? ['Kiểm tra tiến độ', 'Hướng dẫn giấy tờ tiếp theo']
              : ['Cảm ơn bạn!'],
        };
      } catch (error) {
    ...
    if (
      userMessage.toLowerCase().includes('tiến độ') ||
...
Tôi sẵn sàng hỗ trợ bạn!`,
  ...
  private extractUserInfoFromMessage(message: string): any {
    ...
    // Extract name
    const nameMatch =
      message.match(/tên\s+(là\s+)?([a-zA-ZÀ-ÿ\s]+)/i) ||
  ...
  private generateInfoSuggestions(missingInfo: string[]): string[] {
    ...
    if (missingInfo.includes('interestedMajors'))
      suggestions.push('Tôi quan tâm đến ngành...');
    if (missingInfo.includes('budget'))
      suggestions.push('Ngân sách của tôi là...');
    ...
    const majors = [
      {
        ...
        description:
          'Khoa học máy tính với focus vào phát triển phần mềm, AI, và công nghệ',
        careerProspects: [
          'Software Engineer',
          'Data Scientist',
          'Product Manager',
      ...
      },
      {
        ...
        averageSalary: 75000,
        matchScore: this.calculateMajorMatch(
          profile,
  ...
  private calculateMajorMatch(profile: any, major: string): number {
    ...
    // Interest match
    if (
      profile.interestedMajors?.some((interest: string) =>
  ...
  private generateSchoolRecommendations(
    profile: any,
    ...
    const schools = [
      ...
      },
      {
        ...
        ranking: 42,
        tuition: 38000,
        matchScore: this.calculateSchoolMatch(
          profile,
          'University of Texas at Austin',
        ),
        reasons: this.getSchoolReasons(
          profile,
          'University of Texas at Austin',
        ),
      },
  ...
  private calculateSchoolMatch(profile: any, schoolName: string): number {
    ...
    if (profile.budget) {
      if (schoolName.includes('Arizona') && profile.budget >= 30000)
        score += 15;
      if (schoolName.includes('Texas') && profile.budget >= 40000) score += 15;
      if (schoolName.includes('Stanford') && profile.budget >= 60000)
        score += 15;
    }
  ...
  private extractSchoolSelection(
    message: string,
    recommendations: any[],
  ): string | null {
    if (!recommendations) return null;
  ...
  async getConversationState(userId: string): Promise<any> {
    ...
    if (!conversation) {
      return {
        phase: 'introduction',
        message:
          'Chưa có cuộc hội thoại. Hãy bắt đầu bằng cách nói "Xin chào"!',
        isNew: true,
      };
    }
  ...
  async resetConversation(userId: string): Promise<any> {
    ...
    return {
      message:
        'Cuộc hội thoại đã được reset. Chúng ta có thể bắt đầu lại từ đầu!',
      phase: 'introduction',
    };
  }
  ...
  private async generateIntroductionResponse(
    userMessage: string,
  ): Promise<string> {
    const systemPrompt = `
...
`;
  ...
  private async generateInfoCollectionResponse(
    userMessage: string,
    missingInfo: string[],
    extractedInfo: any,
  ): Promise<string> {
...
`;
  ...
  private async generateMajorRecommendationResponse(
    userMessage: string,
    recommendations: any[],
    userProfile: any,
  ): Promise<string> {
...
`;
  ...
  private async generateSchoolRecommendationResponse(
    userMessage: string,
    recommendations: any[],
    selectedMajor: string,
    userProfile: any,
